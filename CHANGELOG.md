...
- **`--check` 的 DPI 诊断修正**：原先报「主屏 DPI 96（100%）」是**失真值**——`--check` 走 headless 分支，没有调用 WinForms 运行时初始化，进程停留在默认 DPI 感知级别，`Graphics.FromHwnd(0)` 把真实 DPI 虚拟化了。实测同一台机器上向导窗口跑在 200% 的屏、`--check` 却报 100%。现在改为在 `Main` 起手就初始化运行时，并**逐显示器**列出实测 DPI 与工作区（多屏不同缩放正是这类问题的高发场景，只报主屏不足以判断向导落在哪块屏、按哪个缩放排版）。
- **`--demo-scale` 的字号换算修正**：点值是物理单位，原先直接乘倍数会与开发机的真实 DPI **叠加**（本机屏是 200%，写 13.5pt 表示"模拟 150%"，实际渲染出 3 倍大）。现在按「目标 DPI ÷ 真实 DPI」折算，于是在任意缩放的开发机上都能准确重现目标缩放的排版。
  - 校准验证：模拟 200% 与真实 100% 相比，客户区、各控件坐标与尺寸、字号全部精确 2.000 倍等比（1120/560、52/26、271/136、126/43）。
