...
- **高分屏（150% / 200% 缩放）下向导界面不可用**：说明文字换行量翻倍、底部被裁，**「开始迁移 / 稍后再说」两个按钮整个被挤出客户区**，用户点不到任何按钮，只能 Alt+F4 关掉窗口。
  - 根因：向导是代码搭的 WinForms 窗体，没有声明 `AutoScaleDimensions`（默认 `(0,0)`，WinForms 会**直接跳过**自动缩放）。于是控件坐标不缩放、字体却按 DPI 放大——文字变多行，把下方内容全挤出去。
  - 修法：布局全部改为按 `DeviceDpi` 手算（`ScaleFactor = DeviceDpi / 96`），窗口高度由内容决定而非写死，按钮尺寸按文字实测自适应，宽高都不写死。
  - 附带保证：按钮始终贴客户区底部定位，即使小屏把窗口夹进工作区，按钮也一定可见可点。
- 日志区在窗口被夹时可能与按钮重叠，已改为「日志只吃剩余空间，放不下就自动收起」。

### 自检

- 新增 `--demo-scale <百分比>`：在 100% 缩放的机器上模拟高分屏排版（如 `--demo-scale 150`），提交前就能拍出高分屏效果。v1.9.10 的问题正是因为开发机是 100%、没有人拍过 150%，才在发布后由用户发现。
- `--check` 增加主屏 DPI 与工作区输出，用户报显示问题时可直接提供现场数据。

## [1.9.10] - 2026-09-22

### 迁移版（本次发布的形态说明）

**这是 ClipboardX(WPF) 的最后一个版本，它的全部内容是一个「迁移向导」。** 装完启动后不再进入旧的剪贴板界面，而是引导你迁移到新一代程序 **[clipx](https://github.com/chaojimct/clipx)**。

- **为什么换**：clipx 是重写版本——界面交互一脉相承，但常驻内存显著更低、启动更快，并补齐了原生 OCR、批量 FIFO/LIFO 等能力。
- **向导做什么**：① 下载并静默安装 clipx（无需管理员权限）；② 停用老版的开机自启，避免两套剪贴板监听并存、热键相撞；③ 启动 clipx，其首次启动会**自动导入你的老版历史与 OCR 结果**。
- **你的数据安全**：向导**不会删除**老版的程序目录，也**不会删除**数据目录（`%LocalAppData%\ClipboardX`，历史库在此）。确认新程序里数据无误后，你可以自行运行老版卸载程序——**卸载时请选「否」保留历史**。
- **老版仍可用**：迁移只是停用开机自启，老版程序原样保留，随时可手动打开。

### 迁移版 launcher（Migrator）

- 新增 `Migrator/`：独立的 WinForms 迁移向导，产物名 `ClipboardX.exe`（与老版一致，才能被更新通道替换）。
- 自检开关：`--check`（只读打印现状）、`--migrate` / `--yes`（无交互跑完整流程，退出码 `0` 干净完成 / `2` 未找到安装包 / `3` 装后无 exe / `4` 自启项未清干净）、`--demo-busy`（以「迁移中」形态打开，供截图核对）。
- 老版管理员模式下注册的自启任务（`RunLevel=HighestAvailable`）普通权限删不掉，向导会**如实报告并给出指引**，不假装已解决。

### 发布包

- 本版只提供 `no-runtime` 与 `self-contained` 两个变体，包内均只有一个 `ClipboardX.exe`（迁移向导）。原先的完整版/精简版三个 flavor 不再更新。

## [1.9.9] - 2026-09-02

### Explorer 快速搜索

- **修复 F2 重命名输入被「输入即搜索」劫持**：桌面 / 资源管理器按 F2 重命名时输入字符误触发 Everything 快速搜索会话，会话吞掉全部按键导致重命名无法输入。三处缺陷叠加修复：
  - 桌面路径（`Progman`/`WorkerW`）此前完全未做焦点编辑框检查（Explorer 窗口路径有检查），补上相同检查——重命名时焦点在 Edit 上不启动会话
  - F2 按下后 Explorer 移焦点到重命名编辑框有延迟，用户立即输入首字符时焦点可能仍在文件列表而误判。现 F2 后 1.5 秒内同一窗口的字符不启动会话，把输入留给重命名框
  - 会话已启动后焦点落入可编辑控件（重命名框迟到就绪、用户点了地址栏/搜索框）时立即结束会话并还键，不再持续吞键
  - Win11 DirectUI 下 `hwndFocus` 偶发为 0 的场景，用 `hwndCaret`/`rcCaret` 兜底判定是否正在编辑文本

## [1.9.8] - 2026-09-02

### 文件跳转 · 稳定性（重点修复）

- **修复跳转数次后宿主程序崩溃/闪退（严重）**：文件对话框的原生注入导航中，`CWM_GETISHELLBROWSER` 返回的 `IShellBrowser` 是对话框**借用**的指针（未 `AddRef`），v1.9.7 内部改动误将其当泄漏补上 `Release()`，导致每次读/导航把宿主 ShellBrowser 引用计数非法 -1，几次操作后对象被提前销毁，宿主（Edge 保存对话框子进程、微信等）随即 fail-fast 崩溃。恢复借用指针语义（不 `Release`）并加注释防止回归
- **修复跳转数次后宿主程序卡死**：注入的导航代码运行在宿主对话框 UI 线程的 `WH_GETMESSAGE` 钩子回调内，busy 重试的 `Sleep` 会把宿主整个消息泵堵死。native 侧改为单次尝试，busy（0x800700AA）重试移至 C# 后台线程（重新拉起远程线程，渐进退避 0~1.2s），宿主线程全程不被阻塞
- **修复并发注入互相干扰**：采集轮询与点击采集并发读、或读+导航并发时会互相覆盖注入 DLL 的全局钩子上下文，导致超时失败甚至句柄误用。native 侧加进程内互斥将钩子式读/导航串行化；超时路径不再关闭可能被迟到回调使用的事件句柄

### 文件跳转 · 安全性

- **自动同步等无人值守跳转不再模拟键盘（防误关对话框）**：对话框打开自动首跳、前台自动跳转、点击后自动跳转、粘性面板自动刷新改为**仅注入导航**——此前注入失败会回退 Ctrl+L + 盲打路径 + Enter，焦点不在地址栏时 Enter 会直接点中「打开/保存」关掉用户正在操作的对话框
- **手动跳转地址栏回退加焦点安全闸**：`GetGUIThreadInfo` 校验焦点必须位于对话框上半部的可编辑控件（地址栏），否则中止键入，杜绝 Enter 误触「打开/保存」
- **地址栏填充改静默方式**：优先 `WM_SETTEXT` 直接填充地址栏再回车，替代逐字键入——后者会触发地址栏自动完成下拉列表弹出（微信等宿主表现明显）

### WPS 兼容

- **修复保存确认框/错误弹窗被误判为文件对话框**：WPS 进程内 `#32770` 一律归入 WPS 对话框，导致自动跳转向保存确认框等消息框发送按键（Enter 直接点中「保存」）。现在 `#32770` 要求标题像文件对话框或子控件含地址栏/文件名输入/Shell 视图特征；Qt 窗口增加形态校验（可见、已启用、非辅助窗口类、尺寸 ≥360×240）

### 开机自启

- **修复登录时闪现命令行黑框**：`schtasks /TR "\"路径\""` 会把引号原样写进任务的 Command 字段，任务计划程序改由 cmd.exe 解释执行导致闪 conhost。改为标准 XML 导入注册（Command 为纯路径）；同时修正任务默认 72 小时执行时限（常驻应用会被强杀）、电池模式禁止启动、任何用户登录均触发三个隐藏问题。升级后首次启动会自动重注册修复旧任务
- **开发版与正式版自启任务隔离**：Debug 构建任务名加 `_Dev` 后缀，避免开发版用 `/F` 覆盖正式版的自启任务

### 其他

- 前台切换到/切出资源管理器时的路径采集（COM 枚举 + UIA 回退可达数百毫秒）移到后台线程，消除 UI 卡顿
- 跳过需要剪贴板通信的文件管理器（XYplorer / Total Commander）前台采集，避免覆盖用户剪贴板
